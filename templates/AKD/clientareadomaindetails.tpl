...
<div id="tabs">
  <ul class="nav nav-tabs">
    ...
    {/if}{/if}
    {if $releasedomain}
    <li id="tab5nav"><a href="#tab5">{$LANG.domainrelease}</a></li>
    {/if}
    {if $addonscount}
    <li id="tab6nav"><a href="#tab6">{$LANG.clientareahostingaddons}</a></li>
    {/if}
    ...
    <li class="dropdown"><a data-toggle="dropdown" href="#" class="dropdown-toggle">{$LANG.domainmanagementtools}&nbsp;<b class="caret"></b></a>
      <ul class="dropdown-menu">
        ...
        {/if}
        {if $emailforwarding}
        <li><a href="clientarea.php?action=domainemailforwarding&domainid={$domainid}">{$LANG.clientareadomainmanageemailfwds}</a></li>
        {/if}
        {if $getepp}
        <li class="divider"></li>
  ...
  <div class="row">
    <div class="col30">
      <div class="internalpadding">
        ...
        <br />
        <p>
          <input type="button" value="{$LANG.backtodomainslist}" class="btn" onclick="window.location='clientarea.php?action=domains'" />
        </p>
      </div>
    </div>
    <div class="col70">
      <div class="internalpadding">
        <div class="row">
          <div class="col2half">
            <h4><strong>{$LANG.clientareahostingdomain}:</strong></h4>
            {$domain} <span class="label {$rawstatus}">{$status}</span> </div>
          <div class="col2half">
            <h4><strong>{$LANG.firstpaymentamount}:</strong></h4>
            <span>{$firstpaymentamount}</span> </div>
        </div>
        <div class="row">
          <div class="col2half">
            <h4><strong>{$LANG.clientareahostingregdate}:</strong></h4>
            <span>{$registrationdate}</span> </div>
          <div class="col2half">
            <p>
            <h4><strong>{$LANG.recurringamount}:</strong></h4>
            {$recurringamount} {$LANG.every} {$registrationperiod} {$LANG.orderyears}{if $renew} &nbsp; <a href="cart.php?gid=renewals" class="btn btn-mini">{$LANG.domainsrenewnow}</a>{/if}
            </p>
          </div>
        </div>
        <div class="row">
          <div class="col2half">
            <p>
            <h4><strong>{$LANG.clientareahostingnextduedate}:</strong></h4>
            {$nextduedate}
            </p>
          </div>
          <div class="col2half">
            <p>
            <h4><strong>{$LANG.orderpaymentmethod}:</strong></h4>
            {$paymentmethod}
            </p>
          </div>
        </div>
    ...
    <div class="col70">
      <div class="internalpadding">
        ...
        <form method="post" action="{$smarty.server.PHP_SELF}?action=domaindetails">
          ...
          <p>
            <input type="submit" class="btn btn-danger" value="{$LANG.domainreglockdisable}" />
          </p>
          {else}
          <p>
            <input type="submit" class="btn btn-success" name="reglock" value="{$LANG.domainreglockenable}" />
          </p>
          {/if}
        </form>
      </div>
...
<div data-toggle="tab" id="tab6" class="tab-content">
  <div class="row">
    ...
    <div class="col70">
      <div class="internalpadding"> {if $addons.idprotection}
        <div class="row">
          <div class="domaddonimg"> <img src="images/idprotect.png" /> </div>
          <div class="col70"> <strong>{$LANG.domainidprotection}</strong><br />
            {$LANG.domainaddonsidprotectioninfo}<br />
            {if $addonstatus.idprotection} <a href="clientarea.php?action=domainaddons&id={$domainid}&disable=idprotect&token={$token}">{$LANG.disable}</a> {else} <a href="clientarea.php?action=domainaddons&id={$domainid}&buy=idprotect&token={$token}">{$LANG.domainaddonsbuynow} {$addonspricing.idprotection}</a> {/if} </div>
        </div>
        <br />
        {/if}
        {if $addons.dnsmanagement}
        <div class="row">
          <div class="domaddonimg"> <img src="images/dnsmanagement.png" /> </div>
          <div class="col70"> <strong>{$LANG.domainaddonsdnsmanagement}</strong><br />
            {$LANG.domainaddonsdnsmanagementinfo}<br />
            {if $addonstatus.dnsmanagement} <a href="clientarea.php?action=domaindns&domainid={$domainid}">{$LANG.manage}</a> | <a href="clientarea.php?action=domainaddons&id={$domainid}&disable=dnsmanagement&token={$token}">{$LANG.disable}</a> {else} <a href="clientarea.php?action=domainaddons&id={$domainid}&buy=dnsmanagement&token={$token}">{$LANG.domainaddonsbuynow} {$addonspricing.dnsmanagement}</a> {/if} </div>
        </div>
        <br />
        {/if}
        {if $addons.emailforwarding}
        <div class="row">
          <div class="domaddonimg"> <img src="images/emailfwd.png" /> </div>
          <div class="col70"> <strong>{$LANG.domainemailforwarding}</strong><br />
            {$LANG.domainaddonsemailforwardinginfo}<br />
            {if $addonstatus.emailforwarding} <a href="clientarea.php?action=domainemailforwarding&domainid={$domainid}">{$LANG.manage}</a> | <a href="clientarea.php?action=domainaddons&id={$domainid}&disable=emailfwd&token={$token}">{$LANG.disable}</a> {else} <a href="clientarea.php?action=domainaddons&id={$domainid}&buy=emailfwd&token={$token}">{$LANG.domainaddonsbuynow} {$addonspricing.emailforwarding}</a> {/if} </div>
        </div>
        {/if} </div>
    </div>
  </div>
