...
<form method="post" action="domainchecker.php" class="form-horizontal">
  <div class="well">
    ...
    <div class="textcenter">
      <div align="right" class="multitldbtn">
    ...
    <div class="domcheckertldselect hide" id="tlds"> {foreach from=$tldslist key=num item=listtld}
      <div class="col4 textcenter">
        <label class="full">
          <input type="checkbox" name="tlds[]" value="{$listtld}"{if in_array($listtld,$tlds) || !$tlds && $num==1} checked{/if}>
          {$listtld}</label>
      </div>
      {/foreach}
      <div class="clear"></div>
    </div>
    <div class="textcenter"> {if $capatacha}
...
<div class="center80">
  ...
  <div class="table_chart">
    <table class="table table-striped table-framed">
      ...
      <tr>
        <td class="textcenter">{if $result.status eq "available"}
        ...
        <td class="textcenter">{if $result.status eq "unavailable"}<a href="http://{$result.domain}" target="_blank">WWW</a> <a href="#" onclick="popupWindow('whois.php?domain={$result.domain}','whois',650,420);return false">WHOIS</a>{else}
