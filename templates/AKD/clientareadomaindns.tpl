...
<br />
{if $error}
<div class="alert alert-error textcenter"> {$error} </div>
{/if}
...
<form class="form-horizontal" method="post" action="{$smarty.server.PHP_SELF}?action=domaindns">
  ...
  <div class="table_chart">
    <table class="table table-striped table-framed">
      ...
      <tr>
        <td><input type="hidden" name="dnsrecid[]" value="{$dnsrecord.recid}" />
          <input type="text" name="dnsrecordhost[]" value="{$dnsrecord.hostname}" size="10" class="input-small" /></td>
        ...
        <td>{if $dnsrecord.type eq "MX"}
          ...
          *{else}
          ...
          {$LANG.domainregnotavailable}{/if}</td>
      </tr>
      {/foreach}
      <tr>
        ...
        <td><input type="text" name="dnsrecordpriority[]" size="2" class="input-small" />
          *</td>
      </tr>
        </tbody>
