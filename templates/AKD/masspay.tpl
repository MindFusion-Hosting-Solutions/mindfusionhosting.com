...
<form method="post" action="clientarea.php?action=masspay" class="form-horizontal">
  ...
  <div class="table_chart">
    <table class="table table-striped table-framed">
      ...
      <tr>
        <td colspan="2"><strong>{$LANG.invoicenumber} {$invid}</strong>
          <input type="hidden" name="invoiceids[]" value="{$invid}" /></td>
      </tr>
  ...
  <div class="center80">
    ...
    <label class="radio inline">
      <input type="radio" class="radio inline" name="paymentmethod" value="{$gateway.sysname}"{if $gateway.sysname eq $defaultgateway} checked{/if} />
      {$gateway.name} </label>
    {/foreach} <br />
    <br />
    <p align="center">
