...
<div id="tabs">
  <ul class="nav nav-tabs" data-tabs="tabs">
    ...
    <li id="tab2nav"><a href="#tab2" data-toggle="tab">{$LANG.serverchangepassword}</a></li>
    {/if}
    {if $downloads}
    <li id="tab3nav"><a href="#tab3" data-toggle="tab">{$LANG.downloadstitle}</a></li>
    {/if}
    ...
    <li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" href="#">{$LANG.productmanagementactions}</a>
  ...
  <div class="row">
    <div class="col30">
      <div class="internalpadding">
        ...
        <br />
        <p>
          <input type="button" value="{$LANG.backtoserviceslist}" class="btn" onclick="window.location='clientarea.php?action=products'" />
        </p>
      </div>
    </div>
    <div class="col70">
      <div class="internalpadding">
        ...
        {$groupname} - {$product} <span class="label {$rawstatus}">{$status}</span>{if $domain}
        <div><a href="http://{$domain}" target="_blank">{$domain}</a></div>
        {/if}
        </p>
        {if $dedicatedip}
        <p>
        ...
        <div class="col2half">
          ...
          <span>{if $configoption.optiontype eq 3}{if $configoption.selectedqty}{$LANG.yes}{else}{$LANG.no}{/if}{elseif $configoption.optiontype eq 4}{$configoption.selectedqty} x {$configoption.selectedoption}{else}{$configoption.selectedoption}{/if}</span>
          </p>
        </div>
        {/foreach}
        ...
        <form method="post" action="{$smarty.server.PHP_SELF}?action=productdetails#tab2">
          ...
          <fieldset class="onecol">
            ...
            <div class="control-group">
              ...
              <div class="controls"> {$username}{if $password} / {$password}{/if} </div>
            </div>
            {/if}
            <div class="control-group">
            ...
            <div class="control-group">
              ...
              <div class="controls"> {include file="$template/pwstrength.tpl"} </div>
            </div>
          </fieldset>
...
<div data-toggle="tab" id="tab3" class="tab-content">
  <div class="row">
    ...
    <div class="col70">
      <div class="internalpadding"> {foreach from=$downloads item=download}
        <p>
        <h4>{$download.title} - <a href="{$download.link}">{$LANG.downloadname}</a></h4>
  ...
  <div class="row">
    <div class="col30">
      <div class="internalpadding">
        ...
        <br />
        {if $addonsavailable}
        <p><a href="cart.php?gid=addons&pid={$id}">{$LANG.orderavailableaddons}</a></p>
        {/if} </div>
    </div>
    <div class="col70">
      <div class="internalpadding">
