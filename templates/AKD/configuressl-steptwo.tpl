...
<form class="form-horizontal" method="post" action="{$smarty.server.PHP_SELF}?cert={$cert}&step=3">
  ...
  <fieldset>
    <div class="control-group">
      ...
      <div class="controls"> {foreach from=$approveremails item=approveremail key=num}
        <div class="col2half">
          <label class="full control-label">
            <input type="radio" class="radio inline" name="approveremail" value="{$approveremail}"{if $num eq 0} checked{/if} />
            <span>{$approveremail}</span></label>
        </div>
        {/foreach} </div>
    </div>
  </fieldset>
