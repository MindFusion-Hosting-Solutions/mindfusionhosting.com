...
<p align="right"><a href="domainchecker.php">{$LANG.domainsimplesearch}</a> | <a href="domainchecker.php?search=bulkregister">{$LANG.domainbulksearch}</a></p>
{/if}
...
<div class="center80">
  ...
  <div class="table_chart">
    <table class="table table-striped table-framed">
      ...
      <tr>
        <td class="textcenter">{if $result.status eq "unavailable"}
        ...
        <td class="textcenter">{if $result.status eq "unavailable"}
          <select name="domainsregperiod[{$result.domain}]">
            {foreach key=period item=regoption from=$result.regoptions}{if $regoption.transfer}
            <option value="{$period}">{$period} {$LANG.orderyears} @ {$regoption.transfer}</option>
            {/if}{/foreach}
          </select>
          {/if}</td>
      </tr>
