...
<br />
<p>
  <input type="button" value="{$LANG.clientareabacklink}" onclick="window.location='clientarea.php?action=productdetails&id={$id}'" class="btn btn-large" />
</p>
{else}
  ...
  <table class="table table-striped table-framed">
    ...
    <tr>
      ...
      <form method="post" action="{$smarty.server.PHP_SELF}">
        ...
        <td class="textcenter"> {if $upgradepackage.pricing.type eq "free"}
          {$LANG.orderfree}<br />
...
{if $upgradepackage.pricing.triennially}
          ...
          {/if} </td>
        <td class="textcenter"><input type="submit" value="{$LANG.upgradedowngradechooseproduct}" class="btn" />
      </form>
        </td>
    </tr>
    {/foreach}
  ...
  <div class="table_chart">
    <table class="table table-striped table-framed">
      ...
      <tr>
        ...
        <td class="textcenter"> {if $configoption.optiontype eq 1 || $configoption.optiontype eq 2}
...
{if $option.selected}
            ...
            {else}
            <option value="{$option.id}">{$option.nameonly} {$option.price}{/if}</option>
