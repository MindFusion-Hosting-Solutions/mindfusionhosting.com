...
<div class="table_chart">
  <table class="table table-striped table-framed">
    ...
    {if $type eq "package"}
    <tr>
      <td><input type="hidden" name="pid" value="{$upgrade.newproductid}" />
        <input type="hidden" name="billingcycle" value="{$upgrade.newproductbillingcycle}" />
        {$upgrade.oldproductname} => {$upgrade.newproductname}</td>
...
<form method="post" action="{$smarty.server.PHP_SELF}">
  ...
  <p align="center"><strong>{$LANG.orderpromotioncode}:</strong> {if $promocode}
    ...
    <input type="submit" value="{$LANG.orderpromovalidatebutton}" class="btn" />
    {/if}</p>
</form>
<br />
<form method="post" action="{$smarty.server.PHP_SELF}">
  ...
  <p>{foreach key=num item=gateway from=$gateways}
    <label style="width:auto;padding-right:10px;">
      <input type="radio" class="radio inline" name="paymentmethod" value="{$gateway.sysname}" {if $selectedgateway eq $gateway.sysname} checked{/if}>
      {$gateway.name} </label>
    {/foreach}</p>
  <div class="clear"></div>
  <br />
