...
{include file="$template/pageheader.tpl" title=$LANG.ticketfeedbacktitle|cat:' #'|cat:$tid} <br />
<br />
...
<br />
<p class="textcenter">
  <input type="button" value="Return to Client Area" onclick="window.location='clientarea.php'" class="btn" />
</p>
<br />
<br />
<br />
...
<br />
<p class="textcenter">
  <input type="button" value="{$LANG.returnclient}" onclick="window.location='clientarea.php'" class="btn" />
</p>
<br />
<br />
<br />
...
<form method="post" action="{$smarty.server.PHP_SELF}?tid={$tid}&c={$c}&feedback=1">
  ...
  <div class="ticketfeedbackstaffcont">
    ...
    <div class="ticketfeedbackrating">
      ...
      <div class="rate">
        <input type="radio" name="rate[{$staffid}]" value="{$rating}"{if $rate.$staffid eq $rating} checked{/if} />
      </div>
      {/foreach}
    ...
    <p class="textcenter">
      <textarea name="comments[{$staffid}]" rows="4" style="width:80%;">{$comments.$staffid}</textarea>
    </p>
  </div>
  {/foreach} <br />
