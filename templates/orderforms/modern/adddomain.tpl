...
<form onsubmit="checkavailability();return false" class="form-inline">
<div class="domainreginput">www. <input class="form-control" type="text" name="sld" id="sld" size="25" value="{$sld}" /> <select name="tld" id="tld" class="form-control" >
{foreach key=num item=listtld from=$tlds}
<option value="{$listtld}"{if $listtld eq $tld} selected="selected"{/if}>{$listtld}</option>
{/foreach}
</select> <input class="btn btn-primary" type="submit" value=" {$LANG.checkavailability} " /></div>
</form>
