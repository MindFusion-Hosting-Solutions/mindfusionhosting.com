...
<div class="well">
    ...
    <div class="row textcenter">
        ...
        <div class="col-md-3"><input type="button" value="{$LANG.searchmultipletlds} &raquo;" class="btn btn-lg btn-block" onclick="jQuery('#tlds').slideToggle()" /></div>
    </div>
    <div class="domcheckertldselect collapse" id="tlds">
    <div class="row">
        {foreach from=$tldslist key=num item=listtld}
            <div class="col-xs-6 col-sm-2"><label class="full"><input type="checkbox" name="tlds[]" value="{$listtld}"{if in_array($listtld,$tlds) || !$tlds && $num==1} checked{/if}> {$listtld}</label></div>
        {/foreach}
        </div>
        <div class="clear"></div>
    </div>
...
<div class="table-responsive"><table class="table table-striped table-framed">
    <thead>
        <tr>
            <th></th>
