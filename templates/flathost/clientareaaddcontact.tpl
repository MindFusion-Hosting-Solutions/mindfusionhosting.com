...
<div class="form-group">{$LANG.clientareachoosecontact}: <select name="contactid" class="form-control" onchange="submit()">
    {foreach item=contact from=$contacts}
        <option value="{$contact.id}">{$contact.name} - {$contact.email}</option>
    {/foreach}
    <option value="new" selected="selected">{$LANG.clientareanavaddcontact}</option>
    </select> </div>
    <input class="btn btn-success" type="submit" value="{$LANG.go}" />
</div>
</form>
